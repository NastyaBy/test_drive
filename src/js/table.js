...
import { getDepartmentCarsListDate } from './server'
...
const initTable = (departmentCars, logbookCars, selectedDate) => {
  ...
  getDepartmentCarsListDate(selectedDate)
    .then((data) => {
      const carsList = data
      ...
      departmentsList.forEach((item) => {
        ...
        if (cars) {
          ...
          cars.forEach((car) => {
            const foundedLogbookCar = logbookCars.filter((item) => item['UF_ID'] === car.ID)
